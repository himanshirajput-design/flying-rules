...
class RefundPolicyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $airlines->getCollection()->transform(function ($airline) {
            $airline->link = route('refund-policy.show', $airline->slug);
            $airline->image = asset($airline->image);
            return $airline;
        });

        return view('refund-policy.index', ['airlines' => $airlines]);
    }
    ...
    public function show($airline)
    {
        ...
        $airlineData['policy'] = $airlineModel->policies()->where('type', 'refund-policy')->first();
        ...
        foreach($relatedAirlinesRaw as $rel) {
            $relatedAirlines[] = [
                ...
                'link' => route('refund-policy.show', $rel->slug)
            ];
        }
