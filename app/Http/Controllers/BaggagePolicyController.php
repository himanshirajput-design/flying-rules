...
class BaggagePolicyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $airlines->getCollection()->transform(function ($airline) {
            $airline->link = route('baggage-policy.show', $airline->slug);
            $airline->image = asset($airline->image);
            return $airline;
        });

        return view('baggage-policy.index', ['airlines' => $airlines]);
    }
    ...
    public function show($airline)
    {
        ...
        $airlineData['policy'] = $airlineModel->policies()->where('type', 'baggage-policy')->first();
        ...
        foreach($relatedAirlinesRaw as $rel) {
            $relatedAirlines[] = [
                ...
                'link' => route('baggage-policy.show', $rel->slug)
            ];
        }
