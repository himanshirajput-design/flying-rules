...
class FlightChangeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $airlines->getCollection()->transform(function ($airline) {
            $airline->link = route('flight-change.show', $airline->slug);
            $airline->image = asset($airline->image);
            return $airline;
        });

        return view('flight-change.index', ['airlines' => $airlines]);
    }
    ...
    public function show($airline)
    {
        ...
        $airlineData['policy'] = $airlineModel->policies()->where('type', 'flight-change')->first();
        ...
        foreach($relatedAirlinesRaw as $rel) {
            $relatedAirlines[] = [
                ...
                'link' => route('flight-change.show', $rel->slug)
            ];
        }
